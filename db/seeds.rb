...
#			print arr[8] + "\n"
#			print arr[9] + "\n"
			Dnd5eSpell.create(name: arr[0].chomp.downcase,
					  level: arr[1],
					  spell_type: arr[2],
					  cast: arr[3],
					  range: arr[4],
...
File.open(path + class_spell_file) do |f|
		while line = f.gets
			arr = line.split('$')
			if Dnd5eSpell.exists?( :name => arr[0].chomp ) and Dnd5eClass.exists?( :name => arr[1].chomp )
				Dnd5eClassSpell.create(dnd5e_class_id: Dnd5eClass.where(name: arr[1].chomp).take.id,
					               dnd5e_spell_id:Dnd5eSpell.where(name: arr[0].chomp).take.id)
			else
...
File.open(path + archetype_spell_file) do |f|
		while line = f.gets
			arr = line.split('$')
			if Dnd5eSpell.exists?( :name => arr[0].chomp ) and Dnd5eArchetype.exists?( :name => arr[1].chomp )
				Dnd5eArchetypeSpell.create(dnd5e_archetype_id: Dnd5eArchetype.where(name: arr[1].chomp).take.id,
					               dnd5e_spell_id:Dnd5eSpell.where(name: arr[0].chomp).take.id)
			else
				print arr[0].chomp + " wasn't found for " + arr[1].chomp + ".\n"
			end
